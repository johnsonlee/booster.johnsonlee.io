...
Regardless of the compression effect and usability, the performance of [pngquant](https://pngquant.org/) is quite amazing, but there is a big problem to use it. Although [pngquant](https://pngquant.org/) is open source software, it is released under the [GPL License](https://www.gnu.org/licenses/gpl-3.0.txt), and Booster is released under the [Apache License](https://www.apache.org/licenses/LICENSE-2.0). How to avoid legal risks while using excellent GPL Licensed software?

## Command SPI

To solve the open source license problem, *Booster* provide the [Command SPI](https://github.com/didi/booster/tree/master/booster-command) to split the compression into two parts -- The API part which managed by Booster, and the implementation part which provided by others, the API is released under the [Apache License](https://www.apache.org/licenses/LICENSE-2.0), but for the implementation, it can be released under the [GPL License](https://www.gnu.org/licenses/gpl-3.0.txt), so, this prblem has been solved perfectly by using the [SPI (Service Provider Interface)](https://docs.oracle.com/javase/tutorial/sound/SPI-intro.html).

## Pngquant Provider

As the provider of the [pngquant](https://pngquant.org/) command line, [booster-pngquant-provider](https://github.com/johnsonlee/booster-pngquant-provider) is released under the [GPL License](https://www.gnu.org/licenses/gpl-3.0.txt) and open sourced separately, currently, it prebuilt both 32-bit and 64-bit executables for Linux, macOS, and Windows platforms.
...
From *AGP 3.0.0*, the *AAPT2* is enabled by default. Since the *AAPT2* uses the [AAPT2 Container format](https://android.googlesource.com/platform/frameworks/base/+/master/tools/aapt2/formats.md) (the file suffix is `.flat`), it needs to determine whether the `android.enableAapt2` option is enabled in the Android project before compressing, so, Booster divides it into different tasks by the AAPT2 options:
...
When using [booster-task-compression-pngquant](https://github.com/didi/booster/blob/master/booster-task-compression-pngquant), the [booster-pngquant-provider](https://github.com/johnsonlee/booster-pngquant-provider) is also required unless the [pngquant](https://pngquant.org/) command-line tool has already been installed, otherwise, it won't work.
...
| Option                                             | Description                       |
| -------------------------------------------------- | ----------------------------------|
| `booster.task.compression.pngquant.option.quality` | Compression quality (default: 80) |
| `booster.task.compression.pngquant.option.speed`   | Compression speed (default: 3)    |
...
1. The resource path after merged
1. The file size after compression
1. The file size before compression
1. The source path
